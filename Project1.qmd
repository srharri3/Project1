...
#vectorize if/else to create new fields in ht
ifelse(
  ht$JWAP == "N/A (not a worker; worker who worked from home)",
  ht$revised <- NA,
  { 
    ht$ltime <- word(ht$JWAP,1)
    ht$lampm <- word(ht$JWAP,2)
    ht$revised <- 'x'
...
for (i in nrow(ht)) {
  if (ht$JWAP[i] == "N/A (not a worker; worker who worked from home)") {
    print(ht$JWAP[i])
    ht$JWAP[i] <- NA
    print(ht$JWAP[i])
  }
    ...
    #ltimepm <- ifelse(lampm=="p.m.",12,0)
    
    rtime <- word(ht$JWAP[i],4)
    rampm <- word(ht$JWAP[i],5)
    #rtimepm <- ifelse(rampm=="p.m.",12,0)
    
    lhour <- as.numeric(word(ltime,1,sep = ":")) #+ lampm #need to add the lampm?
    lminute <- as.numeric(word(ltime,2,sep = ":")) #+ rampm
    
    rhour <- as.numeric(word(rtime,1,sep = ":")) #need to add the lampm?
