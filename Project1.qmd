...
```{r}
plot.census <- function(test_results_formatted, numVars, catVars) {
  numVars <- select(test_results_formatted, where(is.numeric), -PWGTP) %>% colnames()
  categorical_vars <- select(test_results_formatted, where(is.factor) | where(is.character)) %>% colnames()

  ggplot(data = test_results_formatted, mapping = aes(x = get(catVars)), y = get(numVars), weight = "PWGTP") + geom_boxplot()
}
```

Testing the boxplot function.
```{r}
plot.census(test_results_formatted,"GASP","SEX")
```
