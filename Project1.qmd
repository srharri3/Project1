...
1.  Write a *helper* function to take the return from `GET()` and modify it into a `tibble`.
1.  Write a function to query API, with editable parameters:
    1.  Year of survey
        -   Default:
            -   2022
        -   Valid:
            -   Integer in \[2010,2022\]
    1.  Numeric var(s) to return
        -   Default:
            -   AGEP
            -   PWGTP (req)
        -   Valid:
    ...
    1.  Geography level
        ...
        -   Conditions:
            -   Ensure valid user entries.
    1.  Subset argument (to adjust API call made)
        -   Conditions:
            -   User can specify specific Regions, Divisions, or States.
            -   Only specific levels selected will be returned.
            -   Ensure valid user entries.
1.  Write a function to allow user to specify multiple years of survey data **and** all other options above.
    -   Conditions:
