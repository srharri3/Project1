...
plot_census <- function(test_results_formatted, numVars, catVars) {
 ...
 if (!is.factor(test_results_formatted[[catVars]]) && !is.character(test_results_formatted[[catVars]])) {
   stop("Categorical variable must be either a character or a factor.")
 }
  if (!is.numeric(test_results_formatted[[numVars]])) {
    stop("Numeric variable must be numeric.")
  }
  
  test_results_na <- test_results_formatted[!is.na(test_results_formatted[[numVars]]) & !is.na(test_results_formatted[[catVars]]), ]
