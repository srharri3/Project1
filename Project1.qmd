...
#         print(i)
#           #adjust NAs accordingly
#           if (word(temp_join$value[i],1) == "N/A") {
#             temp_join$value_new[i] <- NA
#         
#           } else {
#         
#              ltime <- word(temp_join$value[i],1)
#              lampm <- word(temp_join$value[i],2)
#              
#              rtime <- word(temp_join$value[i],4)
#             rampm <- word(temp_join$value[i],5)
#             
#             lhour <- as.numeric(word(ltime,1,sep = ":")) 
#             lminute <- as.numeric(word(ltime,2,sep = ":")) 
#             
#             rhour <- as.numeric(word(rtime,1,sep = ":")) 
#             rminute <- as.numeric(word(rtime,2,sep = ":"))
#             
#             diffhour <- rhour-lhour
#             diffminute <- rminute-lminute
#             
#             temp_join$value_new[i] <- paste(lhour+floor(diffhour/2),
#                                       ":",
#                                       if (lminute+floor(diffminute/2) < 10)
#                                         "0",
#                                       lminute+floor(diffminute/2),
#                                       " ",
#                                       lampm,
#                                       sep = '',
#                                       collapse = '')
...
```{r}
sample_rows <- c(1,2,3)
time_sample <- matrix(data=c("N/A (not a worker; worker who worked from home)",
                            "4:00 p.m. to 4:04 p.m.",
                            "8:45 a.m. to 8:49 a.m."),
                      nrow=3,
                      ncol=1,
                      byrow=TRUE,
                      dimnames = list(sample_rows,"JWAP")) |> as_tibble()
#as_tibble(

#make a temp tibble; edit to have right colname
ht <- as_tibble(time_sample$JWAP)
names(ht) <- "JWAP"
ht$JWAP_new=0
...
for (i in 1:nrow(ht)) {
  ...
  #adjust NAs accordingly
  if (word(ht$JWAP[i],1) == "N/A") {
    ht$JWAP[i] <- NA

  } else {

    ltime <- word(ht$JWAP[i],1)
    lampm <- word(ht$JWAP[i],2)
    
    rtime <- word(ht$JWAP[i],4)
    rampm <- word(ht$JWAP[i],5)
    ...
    ht$JWAP_new[i] <- stri_paste(lhour+floor(diffhour/2),
                                 ":",
                                 if (lminute+floor(diffminute/2) < 10)
                                   "0",
                                 lminute+floor(diffminute/2),
...
#add NAs
#just prep everything and then, IFELSE at end for NA vs new value

#ht$a1 <- ifelse(word(ht$JWAP,1) == "N/A",NA,ht$JWAP)
...
#tt$final <- if_else(word(tt$value,1) == "N/A",
#                    NA,
#                    {
#                      stri_paste(tt$lhour+floor(tt$diffhour/2),
#                                 ":",
#                                 ifelse(tt$lminute+floor(tt$diffminute/2) < 10,"0",""),
#                                 tt$lminute+floor(tt$diffminute/2),
#                                 " ",
#                                 tt$lampm,
#                                 sep="",
#                                 collapse = ""
...
#
#    
#    ht$JWAP_new[i] <- paste(lhour+floor(diffhour/2),
#                            ":",
#                            if (lminute+floor(diffminute/2) < 10)
#                              "0",
#                            lminute+floor(diffminute/2),
...
```{r}
summary_result <- summary.census(test_results_formatted,"JWMNP", "SEX")
```
...
Coercing character values to be factors for the boxplot.
```{r}
test_results_formatted <- test_results_formatted |>
...
plot.census <- function(df = test_results_formatted, numVars, catVars) {
  ...
  categorical_vars <- names(select(test_results_formatted, where(is.factor)))
...
```{r}
plot.census(df = test_results_formatted, numVars = "JWMNP", catVars ="SEX")
```
