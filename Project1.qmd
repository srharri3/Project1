...
This final function will Write a function to allow user to specify multiple years of survey data **and** all other options above.
...
Creating the plot for the test_results_formatted tibble.
```{r}
plot.census <- function(test_results_formatted, numeric_variables, cat_variables) {
  
   if (!(numeric_variables %in% colnames(test_results_formatted))) {
    stop("Numeric variable does not exist.")
  }
  if (!(cat_variables %in% colnames(test_results_formatted))) {
    stop("Categorical variable does not exist.")
  }
  
  boxplot_census <- ggplot(data = test_results_formatted, aes(x = get(cat_variables), y = get(numeric_variables), weight = PWGTP)) + geom_boxplot()
    
  return(print(boxplot_census))
}
```

Testing the boxplot function.
```{r}
plot.census(test_results_formatted, "AGEP", "SEX")
```
