...
```{r}
summary.census <- function(test_results_formatted, numeric_variables = NULL, cat_variables = NULL) {
 if(is.null(numeric_variables)) {
  numeric_variables <- setdiff(numeric_variables, "PWGTP")}
  
  if(is.null(cat_variables)) {
    names(select(test_results_formatted, where(~ !is.numeric(.x))))
  }
  
  numeric_summary <-
  test_results_formatted |>
    summarize(across(all_of(numeric_variables),
                     list(mean = ~ sum(.x * PWGTP) / sum(PWGTP),
                          sd = ~ sqrt(sum(.x^2 * PWGTP) / sum(PWGTP) - (sum(.x * PWGTP)/ sum(PWGTP))^2)),
                     .names = "{.col}_{.fn}"))
  
  cat_count <-
  test_results_formatted |>
    drop_na(all_of(cat_variables)) |> 
  group_by(across(all_of(cat_variables))) |>
          summarize(count = n(), .groups = "drop")

results <- list(numeric_summary = numeric_summary, categorial_count = cat_count)
return(results)
}
```

```{r}
final_result <- summary.census(test_results_formatted, ,)
```
...
```{r}
plot_census <- function(census_tibble, num_var, cat_var) {
  ggplot(data = test_results_formatted,
aes(x = get(cat_variables), y = get(num_variables), weight = PWGTP)) + geom_boxplot()
}
```
>>>>>>> origin/main
