...
getLookup <- function(varName, year) {
  ...
  for (varName in all_variables) {
    ...
    for (colName in names(results)) {
      ...
      if (varName == colName) {
        ...
        #for a cate type, swap its contents from index to label 
        if (varName %in% cat_variables) {
          #results |> mutate(varName = as.numeric(results[varName]))
          
          #make a temp join but only keep the lookup
          temp <- results[varName]
          temp_join <- left_join(temp,lookup,by=join_by(varName == varName))
...
```{r}
test_results <- getToTibble(
  queryCensus(yr=2022,catVars=c('SEX','HISPEED'),
              #geoLevel='Division',geoSubset = 2,
              numVars=c('AGEP','PWGTP','GASP','JWAP'))
)
test_results
```
